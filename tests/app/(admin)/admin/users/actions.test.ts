...
describe('setUserStatusAction', () => {
    it('sends the TARGET status, so two clicks land on the same state', async () => {
        await setUserStatusAction(IDLE, form({ userId: '7', status: 'inactive' }));
        await setUserStatusAction(IDLE, form({ userId: '7', status: 'inactive' }));
        ...
        // was.
        expect(userAdminService.setUserStatus).toHaveBeenNthCalledWith(1, 7, 'inactive');
        expect(userAdminService.setUserStatus).toHaveBeenNthCalledWith(2, 7, 'inactive');
    });

    it('names what happened in the message', async () => {
        expect(await setUserStatusAction(IDLE, form({ userId: '7', status: 'active' }))).toEqual({
            status: 'success',
            message: 'Account activated.',
        });

        expect(await setUserStatusAction(IDLE, form({ userId: '7', status: 'inactive' }))).toEqual({
            status: 'success',
    ...
    it('revalidates the users table', async () => {
        await setUserStatusAction(IDLE, form({ userId: '7', status: 'active' }));
    ...
    it.each([
        ...
        ['a status outside the enum', { userId: '7', status: 'banned' }],
    ...
    it('returns the service failure and does not revalidate', async () => {
        ...
        const result = await setUserStatusAction(IDLE, form({ userId: '7', status: 'inactive' }));
