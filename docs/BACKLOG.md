...
stands against the code** — checked 4 September 2026.
...
finished in one PR and it is never "the database layer" — epics are cut by _user value_, not by
technical layer.

**Story** = one vertical slice through every layer (page → action → service → domain → repository →
migration) that a user can _see working_. If a story leaves the app in a state where nothing new is
demonstrable, it is a task, not a story — fold it into the story that needs it.

**Acceptance Criteria** = Given / When / Then. Written _before_ the code, testable by someone who
...
| **N**egotiable  | Does it describe the _outcome_, leaving the implementation open? |
...
> **This note is out of date and is kept only for the history.** It used to read
> that four of these seven boxes could not be satisfied by anyone, for want of
> CI, RLS, a test runner and an ESLint config. All four now exist: CI runs on
> every PR (`.github/workflows/ci.yml`), RLS is enabled on all ten tables with
> policies (`supabase/migrations/20260902204628_securitate_rls.sql`), Vitest and
> Playwright are both installed, and ESLint is configured (SP-122).
>
> The tick is now worth something. What it still does not tell you is _when_ a
> story was marked done: anything shipped before those four landed was ticked
> against a definition that could not be enforced, so an old **Done** in Part 3
> is a weaker claim than a new one.
...
_Nothing else starts until SP-006 is merged._
...
### EPIC 1 — Authentication & Roles · _owner A_
...
### EPIC 3 — Catalog & Question Bank · _owner B_
...
- Name is unique and 2–60 chars, enforced by Zod _and_ the DB constraint
...
- Editing text or answers does not change already-submitted responses' `is_correct` snapshots
  _(this is the D4 guarantee — write the test)_
...
### EPIC 4 — Assessment Generation, Run & Submission · _owner C_
...
**SP-044 · Refresh-safe resume** — `3` · _explicit brief requirement_
...
- _Cut this first if Week 5 is tight_

---

### EPIC 5 — Scoring & Results · _owner C_
...
### EPIC 6 — Learning Plan Recommendation · _owner A_
...
### EPIC 7 — Progress Tracking · _owner A_
...
### EPIC 8 — Admin Dashboard, Search & Filtering · _owner B_
...
### EPIC 9 — AI Features · _one story each — this is the graded differentiator_
...
- Given a submitted assessment, When the plan is generated, Then each item gets an `ai_description`
  explaining _why_ it matters for this student's specific results
...
- Each member can explain their AI feature _and_ one architectural trade-off end to end
...
### EPIC 11 — Baseline (General Knowledge) Assessment · _owner C, content by B_

_Every member who signs up meets an empty dashboard: no level, no score, no plan, and a card
inviting them to pick categories they have no basis for picking. This epic gives them a first
number to stand on._

_It is also the narrowest possible instance of Epics 4 and 5 — one category, a fixed paper, one
attempt, no picker and no level selection — so SP-113 through SP-116 are SP-043/045/046/053 with
every variable nailed down. Build this first and most of both epics comes out with it._
...
- General knowledge _of IT_ — development, databases, networking, security and tooling — rather
...
### Epic 0 — Foundation · **4 of 6, 2 partial**

| Story                                          | State        |                                                                                                                                                                                                                                                                                                                                                                                                                    |
| ---------------------------------------------- | ------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| SP-001 Repo, Next, Tailwind, strict TS         | **Done**     | `strict: true`, `.env.example` committed. Both ACs hold: ESLint runs (SP-122) and `any` fails it — `@typescript-eslint/no-explicit-any` is an error                                                                                                                                                                                                                                                                |
...
| SP-003 Migration 0001 — schema                 | **Not done** | `supabase/migrations/` holds six timestamped files — schema, RLS and four later changes — plus `config.toml` and `seed.sql`. This was the highest-priority item in the backlog for a long time. Naming is the CLI's `<timestamp>_<name>.sql`, not the `NNNN_name.sql` the spec asks for                                                                                                                            |
| SP-004 Migration 0002 — RLS + `answer_options` | **Not done** | RLS is enabled on all ten tables with policies, so AC1 and AC4 hold. **AC2 does not:** `answers` carries one policy, `for select using (true)`, over a blanket `grant all … to anon`, and RLS filters rows rather than columns — `answers.is_correct` is still readable with the publishable key. AC3's `answer_options` view is in no migration, and it is the whole of what is left                              |
| SP-005 CI pipeline                             | **Done**     | `ci.yml` runs on every PR and on push to `main`: format:check -> typecheck -> lint -> test:coverage -> build, cheapest gate first. Lint was reinstated by SP-122 — Next 16 dropped `next lint`, so `npm run lint` is now `eslint .` against a flat config. Branch protection still has to be switched on in the repository settings — the workflow cannot do that for you                                          |
| SP-006 Tracer bullet                           | **Not done** | The path it describes exists and runs on every push: `e2e/baseline-journey.spec.ts` goes from seeded questions to a score on screen and on to a plan, grading server-side through the `grade_assessment()` RPC. It arrived slice by slice rather than as one tracer, which is why the rest of this table stopped being downstream of it. The Vercel-preview acceptance criterion is the one part not verified here |
...
| SP-011 Registration                     | **Partial**  | Creates the `users` row, honours the role selector, seeds `category_progress` from the chosen interests, and rejects a duplicate email _and_ a duplicate first+last name. An admin registration requires a manager-approval checkbox. No `auth.users`, no trigger, no password |
...
### Epic 10 — Quality, Docs & Demo · **4 of 11, 2 partial**
...
| SP-122 Configure ESLint                      | **Done**        | `eslint.config.mjs`, flat config, `eslint-config-next` + `eslint-config-prettier` in that order so the two tools cannot argue about the same line. `npm run lint` is `eslint .` again and CI runs it as its own gate. 269 files linted (`.ts`, `.tsx`, `.mjs`), zero errors; seven `react-hooks/set-state-in-effect` warnings are deliberate and documented in the config. `@typescript-eslint/no-explicit-any` is on as an error, which is what finally makes the ban on `any` in ARCHITECTURE §8 a check rather than a convention — it started green, because there was no `any` in the tree to fix first                             |
...
not. The single missing capability behind Epics 4, 5, 6, 9 and 11 is _taking an
assessment_, and the single missing artefact behind Epic 0 is _the schema_.
