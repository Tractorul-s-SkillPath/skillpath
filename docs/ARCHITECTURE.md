...
**Status:** partly built. §0 says which parts, and is accurate as of 4 September 2026, after the migrations, RLS, Supabase Auth, the test suites and ESLint landed. §7's own status block is the one part not re-checked — it is marked as such where it starts.
...
| Grading never happens in the browser (§5)                | `assessments.grade()` is an RPC to `grade_assessment()`, called from `assessment.repo.ts`. The E2E baseline journey grades a real paper end to end |

### Not true, and why

| §         | The document says                                                                                                              | Actually                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| --------- | ------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| 3, 4.4, 8 | `supabase/migrations/NNNN_name.sql` is the schema                                                                              | **True now.** `supabase/migrations/` holds six timestamped files — the schema, RLS, and four later changes — applied in filename order, plus `config.toml` and `seed.sql`. This was for a long time the largest single gap in the project, and closing it is what makes `tests/db`, the E2E project and a fresh clone possible at all. The naming is the CLI's `<timestamp>_<name>.sql`, not the `NNNN_name.sql` this document specifies. One thing did **not** come with it: `lib/supabase/database.types.ts` is still hand-written rather than generated by `supabase gen types`, so it can drift from the migrations. When the two disagree, the migrations are the schema.                                                                                                                                           |
| 2, 4, 5   | Supabase Auth (GoTrue) owns credentials; `profiles.user_id uuid` references `auth.users(id)`                                   | **Built — this row has flipped.** `public.users.user_id` is a `uuid` primary key referencing `auth.users(id) on delete cascade`, there is no `password` column, and `loginAction` calls `supabase.auth.signInWithPassword` (`app/(auth)/login/actions.ts`). The table is named `users` rather than `profiles`, so §4.4's name still does not match, but the shape does. The scrypt hash/verify pair and the signed cookie of our own are both gone — see the header of `lib/auth/current-user.ts`. Deviation **D1** is applied after all. One consequence is recorded in `20260904090000_signup_role_from_metadata.sql` and worth reading before a demo: `/register` can now create an administrator, gated by a self-declared checkbox, so the signup form is a public path to an account that can read the answer key. |
| 5         | Row Level Security is the real authorization boundary; every table has policies                                                | **Built, and it does not yet do the job §5 asks of it.** RLS is enabled on all ten tables with policies, in `20260902204628_securitate_rls.sql`. Personal data is genuinely scoped — `using (user_id = auth.uid())` on `users`, `assessments` and the rest — so the cross-member reads in §5 are closed. **The answer key is not.** `answers` carries one policy, `for select using (true)`, on top of a blanket `grant all on all tables in schema public to anon` in the schema migration, and RLS filters rows rather than columns: any holder of the publishable key can `GET /rest/v1/answers?select=is_correct`. Closing it needs the `answer_options` view §5 designs, which no migration creates. See the row below.                                                                                             |
...
| 5         | `answers` is revoked from the API and read through an `answer_options` view                                                    | **Not built, and no longer blocked on anything.** `answers.is_correct` is reachable over PostgREST with the publishable key, so the answer key is obtainable. The application layer is careful — student-facing reads go through `question.service` and never select the column — but that care is defeated by a direct request. This used to be blocked behind the RLS decision; RLS has since landed and does not close it, because a policy filters rows and the leak is a column. The view is now the whole of the remaining work, and it is the last unsatisfied acceptance criterion in SP-004.                                                                                                                                                                                                                    |
...
| 7         | Vitest, RTL, Playwright, 75% coverage gate                                                                                     | **Built.** Vitest runs 481 tests across 43 files in the gate (`npm run test:coverage`), well above the 75% threshold — 99% statements overall, 100% on `lib/services` and `lib/auth`. A second suite needs a database (`npm run test:db`) and Playwright runs two journeys (`npm run test:e2e`), each with its own workflow. RTL is the one part not built: neither `@testing-library/react` nor `@vitejs/plugin-react` is installed, so `tests/components` and the `.tsx` specs under `tests/app` cannot run yet. `docs/TESTING.md` is the detail.                                                                                                                                                                                                                                                                      |
| 8, 9      | CI runs typecheck → lint → coverage → build                                                                                    | **Built**, with a format gate in front of it: `.github/workflows/ci.yml` runs `format:check` → `typecheck` → `lint` → `test:coverage` → `build` on every PR and every push to `main`. Lint is ESLint against the flat config in `eslint.config.mjs` (SP-122); Next 16 removed `next lint`, so it invokes `eslint` directly. Errors fail the step and warnings do not — the seven current warnings are documented in that config. Branch protection is still off in the repository settings, so CI reports without blocking.                                                                                                                                                                                                                                                                                              |
...
>
> **The tooling half of this status note is out of date and the rest of it has
> not been re-checked.** `package.json` now has `typecheck`, `format`, `lint`
> and five `test:*` scripts; `.github/workflows/ci.yml` runs on every PR and
> every push to `main`; ESLint is configured (SP-122, `eslint.config.mjs`) and
> its errors fail that pipeline, the ban on `any` in §8 among them. Vitest and
> Playwright are both installed, and the unit suite runs 481 tests across 43
> files in about a second. What is written above about `tests/` being docblocks
> and `e2e/` being absent describes an earlier state of the repository — the
> rest of §7 has not been re-checked against the code.
>
...
- **`any` is banned.** So is disabling a lint rule without a comment saying why. Both halves hold
  today: `@typescript-eslint/no-explicit-any` is an error in `eslint.config.mjs`, so CI rejects the
  first, and both `eslint-disable` comments in the tree carry a reason, which is convention rather
  than a check — no rule enforces the second.
...
- **CI in Week 3, not Week 6:** typecheck → lint → `vitest --coverage` (fails under threshold) →
  `next build`. _(Built, with `format:check` added in front — see `.github/workflows/ci.yml`.
  Branch protection is not switched on, so CI reports without blocking a merge.)_
