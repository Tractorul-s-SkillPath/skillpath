...
export default function QuestionForm({ categoryId }: { categoryId: number }) {
  ...
  const addOption = () => {
    if (optionCount < 4) setOptionCount(prev => prev + 1);
  };

  const removeOption = () => {
    if (optionCount > 2) setOptionCount(prev => prev - 1);
  };
      ...
      <form ref={formRef} action={formAction} className="space-y-4">
        ...
        <div className="space-y-3 pt-2">
          <div className="flex justify-between items-end border-b pb-2">
            <label className="block text-sm font-semibold text-slate-700">Answers (Check correct ones) *</label>
            <div className="flex gap-2">
              <button type="button" onClick={removeOption} disabled={optionCount <= 2} className="px-3 py-1 text-sm font-bold text-slate-500 bg-slate-100 rounded-lg hover:bg-slate-200 disabled:opacity-50 transition-colors">-</button>
              <button type="button" onClick={addOption} disabled={optionCount >= 4} className="px-3 py-1 text-sm font-bold text-slate-500 bg-slate-100 rounded-lg hover:bg-slate-200 disabled:opacity-50 transition-colors">+</button>
            </div>
          </div>
